...
export default function HeroUi({ categories }) {
    return <header>
        ...
        <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            <ul class="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
                {categories.map((category) => {
                    const category_slug = "categories" + {category.slug}
                    return(
                    <li key={category.slug}>
                    <Link href={category_slug} class="block py-2 pr-4 pl-3 text-white rounded bg-primary-700 lg:bg-transparent lg:text-primary-700 lg:p-0 dark:text-white" aria-current="page">{category.name}</Link>
                    </li>
                )}
                )}
            </ul>
